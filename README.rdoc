...
Because we're using multiple OpenIDs, we need a new Login model to replace `openid_identifier`:

  script/generate model Login identifier:string

Next let's modify our User and Login models adding `has_many :logins` and `belongs_to :user`. One more change, we'll add a method called `find_by_openid_identifier` to User, to override how the authlogic code does lookups.
  ...
  class User < ActiveRecord::Base
    ...
    def self.find_by_openid_identifier(identifier)
      Login.find_by_identifier(identifier).user
    end
...
Then open the generated `create_logins.rb` migration and change it to something like:
  ...
  class CreateLogins < ActiveRecord::Migration
    def self.up
      ...
      add_index :logins, :identifier
      add_index :logins, :user_id
      User.find(:all).each do |user|
        unless user.openid_identifier.blank?
          user.logins << Login.new do |l|
            l.identifier = user.openid_identifier
          end
          user.save
        end
      end
    ...
    def self.down
      remove_index :logins, :identifier
...
The above migration should port over all existing OpenIDs. (It worked for me, at least) Just make sure you've made the right changes to Login and User first.
